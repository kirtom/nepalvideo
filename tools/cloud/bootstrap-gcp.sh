...
# The module renders its own unit and touches its own stamp, so nothing the
# watchdog reads is spelled out twice; a failure to install must not stop
# the box from booting.
if ! sudo -u $USER_NAME .venv/bin/python -m nepal.cloud.watchdog install | bash; then
...
sudo -u $USER_NAME .venv/bin/python -m nepal.cloud.watchdog touch
