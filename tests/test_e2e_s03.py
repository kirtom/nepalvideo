...
def test_re_detection_replaces_a_recordings_shots_rather_than_adding_to_them(project):
    ...
    Detection resumes per recording since step 2 -- a recording with shots is
    done -- so a stale shot survives a plain run and goes with `--redo shots`
    (``redo_all``), which is the run a threshold change asks for."""
    from nepal.stages.s03_process import detect_shots
    cfg, conn = project
    ...
    conn.execute("INSERT INTO shots(shot_id, recording_id, media_kind, start_s, end_s) "
                 ...
                 "'video',0.0,1.0)")
    conn.commit()
    detect_shots(cfg, conn)                        # resumes: the recording has shots
    assert conn.execute("SELECT COUNT(*) FROM shots WHERE media_kind='video'"
                        ).fetchone()[0] == before + 1
    detect_shots(cfg, conn, redo_all=True)         # re-detects: the ghost goes
    assert conn.execute("SELECT COUNT(*) FROM shots WHERE media_kind='video'"
                        ).fetchone()[0] == before
