...
### 1.8 End credits (mandatory)

The film ends with credits. They are **not part of the runtime range in §1.6** —
15 to 40 minutes is the film; the credits run after it and are measured
separately, 60 to 120 seconds.

They exist for three reasons, in this order: to credit the people who carried
the cameras up a mountain, to be honest about how the film was made, and to be
the one place in twenty minutes of altitude and weather where the film is
allowed to be funny about itself.

**What runs under them.** The shots that did not make the cut. By the time the
credits are built, `shots` holds well over a thousand candidates and the film
used a couple of hundred; the rest are sitting there already measured, already
positioned, already rejected. Played fast and small, they are the outtakes reel
the trek never shot on purpose. Prefer the ones rejected for `shaky` and
`too short` over `exposure` — a blown-out frame is not funny, a lurching one is.

**Music.** One track is reserved for the credits and withheld from the act
assignment entirely (`music.credits_track`, §S02.7). It is the only cue in the
film chosen by a person for a specific moment rather than assigned by a
Hungarian solve on five normalised features, and the credits should say so.

**Content.** Four blocks, in order:

1. **The people.** Who filmed it, who walked it, where and when. The two face
   clusters S03.6 labels are the two people in the film; the labels are
   confirmed at Gate 2 and the real names belong here.
2. **The machine.** That the edit was assembled by software, what it chose, and
   what it was not allowed to choose. Every frame was shot on the trek: nothing
   here is generated, re-timed or invented, and the credits say that plainly —
   an audience in 2026 will otherwise assume otherwise, and they would be
   wrong.
3. **The tooling.** The real list, not a logo wall: Python, SQLite, ffmpeg,
   exiftool, PySceneDetect, OpenCV, librosa, faster-whisper, pillow-heif, NASA
   SRTM elevation tiles, the GeoNames gazetteer, and Claude Code, which wrote
   the pipeline.
4. **The numbers.** A short card of statistics from the database, generated
   rather than typed, because the funny ones are true and nobody would believe
   them otherwise. Candidates: assets ingested, hours of footage, shots
   considered against shots used, recordings the scene detector found no cut in,
   photographs that needed a decoder installed before they could be opened,
   files that claimed to be 360° video and were not, and the number of days the
   camera's clock was wrong by while nobody on the mountain noticed.

The statistics card is built from `shots`, `assets` and `stage_units` at render
time. A hand-typed number in a credit roll is a number that will be wrong by the
next run.
...
- **`nepal_20min.mp4`** — full timeline, plus credits
- **`nepal_3min.mp4`** — re-run S06's MMR fill with a 180 s budget and the same constraints proportionally scaled; the higher threshold naturally selects the strongest shots. **No credits**: a 60-second roll on a 180-second cut is a third of the film.

Write to `s3://nepal/deliver/`, generate presigned URLs, notify via SNS.

#### S09.1 Credits

Built last, from the database, per §1.8. 60–120 s, appended after the final
timeline slot and **not counted against `film.max_duration_s`**.

1. **Bed.** Shots with `status = 'rejected'`, ordered by act so the outtakes
   still walk the route, at 2–3 s each and no more than one per rejected
   recording. Rank by rejection reason — `shaky` and `too short` first,
   `exposure` last — then by `score_tech` descending within a reason, so the
   reel is the near-misses rather than the unwatchable. Where there are not
   enough, hold the last card longer rather than reaching into the cut.
2. **Cards.** The four blocks of §1.8 over the bed, legible at 540p.
3. **Statistics.** Generated by one query pass at render time; never typed.
4. **Music.** `decisions.credits_track`, from first beat, ducked under nothing —
   there is no dialogue here. Fade on the last card.

The credits are rendered at Gate 3 along with the draft, so they are reviewed
with the film rather than appearing for the first time in the deliverable.
